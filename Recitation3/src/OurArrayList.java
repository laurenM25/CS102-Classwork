...
public class OurArrayList<E> implements List<E> {
  ...
  public OurArrayList(int capacity) { // constructs list with given capacity
  ...
  private class ArrayIterator implements Iterator<E> {
    ...
    public void remove() throws IllegalStateException {
      ...
      OurArrayList.this.remove(j - 1); // that was the last one returned
      j--; // next element has shifted one cell to the left
      removable = false; // do not allow remove again until next is called
    }
  } // ------------ end of nested ArrayIterator class ------------

  // ADDED MATERIAL MAY 28
  // 1) switch first and last elements of list
  public void Switch() {
    if (size() > 1) {
      E temp = data[0];
      data[0] = data[size() - 1];
  ...
  // 2) swap two elements at (unequal) specified indices
